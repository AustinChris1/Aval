 ...
 * @notice Feedback about an agent, keyed by the client that gave it. Self-feedback
 *         is rejected: the caller must not own or be approved for the agent.
 *
 * @dev Port of ReputationRegistryUpgradeable.sol from the ERC-8004 reference
 *      implementation to a non-upgradeable contract; external ABI unchanged.
 *
 *      LETTER's use of this is deliberate: the only client that writes feedback
 *      for a settled job is the LetterOfCredit contract itself, which can only do
 *      so after funds actually moved under a mandate. That makes the score
 *      payment-backed rather than self-asserted — the same intent as the
 *      `proofOfPayment` field in the spec's off-chain feedback format.
 */
contract ReputationRegistry {
    ...
    function giveFeedback(
        ...
        // Also reverts with ERC721NonexistentToken if the agent does not exist.
        require(
            !IIdentityRegistryReputation(_identityRegistry).isAuthorizedOrOwner(msg.sender, agentId),
