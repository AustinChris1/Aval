 ...
 * @notice An agent asks a named validator to attest a piece of work; the
 *         validator answers with a score 0..100 against the request hash.
 *
 * @dev Port of ValidationRegistryUpgradeable.sol from the ERC-8004 reference
 *      implementation to a non-upgradeable contract; external ABI unchanged.
 *
 *      In LETTER this is the documentary examination step: the agent presents
 *      documents, requests validation over the document hash, and the letter can
 *      only be drawn once the named validator has scored that exact hash at or
 *      above the threshold written into the letter at issuance.
 */
contract ValidationRegistry {
    ...
    function validationRequest(
        ...
        IIdentityRegistryValidation registry = IIdentityRegistryValidation(_identityRegistry);
    ...
    /// @notice True only once the named validator has actually answered.
    function hasResponse(bytes32 requestHash) external view returns (bool) {
        return validations[requestHash].hasResponse;
    }
