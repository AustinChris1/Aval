...
> The agent never holds the money, the credit does, and it executes the agent's
...
> reverts, no agent can register, and we do not hold the upgrade key. So AVAL
...
1. **0:00**, Dashboard home. One credit, `Settled`. Point at the deployment
   addresses, all verified.
2. **0:10**, Open the credit replay. Let the timeline sit on screen for a beat:
...
demo credit's own float was 0.5 BOT, of which 0.25 returned to the applicant,
