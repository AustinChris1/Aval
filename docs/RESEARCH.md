...
price: **20 gwei**, flat. At that price a full LETTER deployment cost under
...
So: ERC-8004 is announced on BOT Chain and not usable on BOT Chain. LETTER
...
- **`giveFeedback` rejects self-feedback**: it reverts if the caller owns or is
  approved for the agent. This is why LETTER must *not* hold blanket ERC-721
...
- **A pending validation reads as `response = 0`**, indistinguishable from a
  genuine zero score, and the spec exposes no "has answered" flag. LETTER
...
- The two addresses in the B DEX frontend bundle that look like a router and a
  ...
  `factory()`. LETTER's demo deliberately depends on no external protocol.
